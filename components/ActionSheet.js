...
const statusH = Platform.OS === 'ios' ? 0 : 24;
...
export default class ActionSheet extends Component {
    ...
    componentWillMount() {
        ...
        this.totalHeight = this.real_titleHeight + this.real_itemsPartHeight + this.real_cancelPartHeight + this.state.bottomSpace + statusH;
            ...
            return (
                ...
                >
                    <View style={[styles.container]}>
                        <Animated.View
                            style={[styles.maskViewStyle, {opacity: this.state.maskOpacity}]}></Animated.View>
                        <Animated.View style={[{
                            width: width,
            ...
            return (
                <TouchableWithoutFeedback>
                    <View style={[styles.contentViewStyle, {
                        ...
                        height: this.real_titleHeight,
                        borderTopLeftRadius: this.state.borderRadius,
                        borderTopRightRadius: this.state.borderRadius,
                        padding: this.state.mainTitlePadding
                    }]}>
                        <Text style={{
                            color: this.state.mainTitleColor,
                            fontSize: this.state.mainTitleFont,
                            fontWeight: this.titleFontWeight,
                            textAlign: this.state.mainTitleTextAlign,
                        }}>{this.props.mainTitle}</Text>
                    </View>
                </TouchableWithoutFeedback>
            )
        }
                    ...
                    <TouchableOpacity onPress={this._didSelect.bind(this, i)} activeOpacity={0.9}>
                        <View style={[styles.contentViewStyle, {
                            ...
                            <Text style={[styles.textStyle, {
                                color: this.state.itemTitleColor,
                                fontSize: this.state.itemTitleFont,
    ...
    _renderItemSeperateLine(show) {
        if (show) {
            return (<View style={{
                width: this.contentWidth,
                height: this.state.itemVerticalSpace,
                backgroundColor: this.itemSpaceColor
            }}/>);
        } else {
            return null;
        }
        ...
        return (
            <View style={{width: this.contentWidth, height: this.real_cancelPartHeight}}>
                ...
                <TouchableOpacity onPress={this._dismiss.bind(this)} activeOpacity={0.9}>
                    <View style={[styles.contentViewStyle, {
                        backgroundColor: this.cancelBackgroundColor,
                        borderRadius: this.state.borderRadius,
                        width: this.contentWidth,
                        height: this.state.cancelHeight
                    }]}>
                        <Text style={[styles.textStyle, {
                            color: this.state.cancelTitleColor,
                            fontSize: this.state.cancelTitleFont,
                            fontWeight: this.cancelFontWeight
                        }]}>{this.state.cancelTitle}</Text>
                    </View>
                </TouchableOpacity>
            </View>
        );
    ...
    _renderCancelSeperateLine(show) {
        if (show) {
            return (<View style={{
                width: this.contentWidth,
                height: this.state.cancelVerticalSpace,
                backgroundColor: this.cancelSpaceColor
            }}/>);
        } else {
            return null;
        }
